...
stage.info('© 2014 Lorenzo Ferrante');
stage.info('repostory at : https://github.com/lorenzoferrante/quick-html');

mkdirp('Web-Template', function(err) { 
    if (err) console.error(err)
    else stage.success('Created main directory!')
});

mkdirp('Web-Template/css', function(err) { 
	if (err) console.error(err)
    else stage.success('Created css directory!')
});

mkdirp('Web-Template/js', function(err) { 
	if (err) console.error(err)
    else stage.success('Created js directory!')
});

fs.writeFile('Web-Template/index.html', 
	'<!DOCTYPE HTML>\n<html>\n  <head>\n  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />\n  <title>Your Website</title>\n  <link rel="stylesheet" type="text/css" href="css/index.css">\n <script type="text/javascript" src="http://code.jquery.com/jquery-1.11.1.min.js"></script>\n <script type="text/javascript" src="js/app.js"></script>\n </head>\n<body>\n</body>\n</html>', function (err) {
    if (err) throw err;
    stage.success('Created HTML file!');
});
